...
نسخه جدید تمام ایمیل‌ها و تلفن‌های کشف‌شده را همراه با URL منبع، محل استخراج، زمان و امتیاز اطمینان در `contact_emails` و `contact_phones` نگهداری می‌کند. JSON-LD نیز خوانده می‌شود، مقصد نهایی لینک درخواست ثبت می‌شود و نتیجه بررسی وب‌سایت برای فرصت‌های یک شرکت Cache می‌شود. هیچ کنترل یا محدودیتی براساس کشور و پیش‌شماره تلفن اعمال نمی‌شود.
