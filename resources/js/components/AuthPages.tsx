...
function ResetPage() {
  ...
  return <FormPage title="تعیین رمز جدید" description="رمز جدید خود را وارد کنید.">{success ? <p className="form-success" role="status">{success} <a href="/">ورود به حساب</a></p> : !token || !email ? <p className="form-error" role="alert">لینک بازیابی ناقص است. <a href="/forgot-password">درخواست لینک تازه</a></p> : <form className="stack-form" onSubmit={submit}><label>رمز جدید<input type="password" name="password" minLength={8} required autoComplete="new-password" /></label><label>تکرار رمز جدید<input type="password" name="password_confirmation" minLength={8} required autoComplete="new-password" /></label><button className="primary-button wide" disabled={busy}>{busy ? "در حال ذخیره…" : "ثبت رمز جدید"}</button></form>}{error && <p className="form-error" role="alert">{error}</p>}</FormPage>;
}
