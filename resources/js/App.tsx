...
import { ArrowLeft, BadgeEuro, Bookmark, BriefcaseBusiness, Building2, CalendarDays, Check, ChevronLeft, FileCheck2, FileText, GraduationCap, House, LayoutGrid, ListChecks, LogOut, MapPin, Menu, Mic, Save, Search, Sparkles, Trash2, UserRound, Wrench, X } from "lucide-react";
import { api } from "./api";
...
import LegalPages, { type LegalPageKey } from "./components/LegalPages";
import type { GermanCv, Meta, Opportunity, OpportunityCollection, Profile, SavedSearch, SearchFilters, User } from "./types";

type View = "home" | "search" | "saved" | "details" | "guide" | "eligibility" | "tools" | "interview" | "applications" | "employer" | LegalPageKey;
type Filters = SearchFilters;
const emptyMeta: Meta = { categories: [], cities: [], german_levels: ["A2", "B1", "B2", "C1"], legal: { terms_version: "2026-09-18", privacy_version: "2026-09-18", resume_retention_days: 180, provider: { name: null, legal_form: null, representative: null, street_address: null, postal_code: null, city: null, country: "Deutschland", email: null, phone: null, register_name: null, register_number: null, vat_id: null, supervisory_authority: null, responsible_content: null, data_protection_email: null } } };
...
const legalPaths: Record<LegalPageKey, string> = { privacy: "/privacy", terms: "/terms", impressum: "/impressum", "resume-retention": "/resume-retention" };
function legalPageFromPath(): LegalPageKey | null {
  const path = window.location.pathname.replace(/\/$/, "") || "/";
  return (Object.entries(legalPaths).find(([, value]) => value === path)?.[0] as LegalPageKey | undefined) || null;
}
function isLegalView(view: View): view is LegalPageKey { return view in legalPaths; }
...
export default function App() {
  const [view, setView] = useState<View>(() => legalPageFromPath() || "home"), [meta, setMeta] = useState<Meta>(emptyMeta), [filters, setFilters] = useState<Filters>(initialFilters), [searchDraft, setSearchDraft] = useState("");
  ...
  const [loading, setLoading] = useState(true), [error, setError] = useState(""), [user, setUser] = useState<User | null>(null), [authFeedback, setAuthFeedback] = useState(""), [authOpen, setAuthOpen] = useState(false), [profileOpen, setProfileOpen] = useState(false), [cvOpen, setCvOpen] = useState(false), [coverOpen, setCoverOpen] = useState(false), [mobileMenu, setMobileMenu] = useState(false), [profileCompletion, setProfileCompletion] = useState(0), [cvCompletion, setCvCompletion] = useState(0);
  const [savedSearches, setSavedSearches] = useState<SavedSearch[]>([]), [savedSearchesLoading, setSavedSearchesLoading] = useState(false);
  ...
  const loadOpportunities = useCallback(async () => {
    ...
    try {
      ...
      if (view === "saved" && !user?.email_verified) { setOpportunities([]); setTotal(0); setLastPage(1); return; }
      if (view === "saved") response = await api<OpportunityCollection>(`/favorites?page=${page}`);
  ...
  const loadSavedSearches = useCallback(async () => {
    if (!user?.email_verified) { setSavedSearches([]); return; }
    setSavedSearchesLoading(true);
    try { setSavedSearches((await api<{ data: SavedSearch[] }>("/saved-searches")).data); }
    catch { setSavedSearches([]); }
    finally { setSavedSearchesLoading(false); }
  }, [user]);
  useEffect(() => { if (view === "saved") void loadSavedSearches(); }, [loadSavedSearches, view]);
  const navigate = (next: View) => {
    if (next === "saved" && !user) { setAuthOpen(true); setMobileMenu(false); return; }
    if (next === "saved" && user && !user.email_verified) { window.location.href = "/account"; return; }
    const nextPath = isLegalView(next) ? legalPaths[next] : "/";
    if (window.location.pathname !== nextPath) window.history.pushState({}, "", nextPath);
    setView(next); setMobileMenu(false);
  };
  function setFilter<K extends keyof Filters>(key: K, value: Filters[K]) { setPage(1); setFilters(c => ({ ...c, [key]: value })); }
  function submitSearch(e: FormEvent) { e.preventDefault(); setFilter("q", searchDraft.trim()); navigate("search"); }
  async function toggleFavorite(o: Opportunity) { if (!user) { setAuthOpen(true); return; } if (!user.email_verified) { window.location.href="/account"; return; } const next = !o.is_favorite; setOpportunities(a => view === "saved" && !next ? a.filter(x => x.id !== o.id) : a.map(x => x.id === o.id ? {...x,is_favorite:next}:x)); setSelected(c => c?.id === o.id ? {...c,is_favorite:next}:c); try { await api(`/favorites/${o.slug}`, { method: o.is_favorite ? "DELETE" : "PUT" }); } catch { void loadOpportunities(); setSelected(c => c?.id === o.id ? {...c,is_favorite:o.is_favorite}:c); } }
  async function saveCurrentSearch() {
    if (!user) { setAuthOpen(true); return; }
    if (!user.email_verified) { window.location.href = "/account"; return; }
    const name = window.prompt("برای این جست‌وجو یک نام بنویسید:", filters.city ? `فرصت‌های ${filters.city}` : "جست‌وجوی جدید")?.trim();
    if (!name) return;
    try { await api("/saved-searches", { method: "POST", body: JSON.stringify({ name, filters }) }); window.alert("جست‌وجو ذخیره شد و از بخش «ذخیره‌شده‌ها» در دسترس است."); }
    catch (exception) { window.alert(exception instanceof Error ? exception.message : "ذخیره جست‌وجو ممکن نشد."); }
  }
  function runSavedSearch(search: SavedSearch) { setFilters({ ...initialFilters, ...search.filters, german_level: (search.filters.german_level || "").toUpperCase() }); setPage(1); navigate("search"); }
  async function deleteSavedSearch(search: SavedSearch) { if (!window.confirm(`جست‌وجوی «${search.name}» حذف شود؟`)) return; try { await api(`/saved-searches/${search.id}`, { method: "DELETE" }); setSavedSearches(items => items.filter(item => item.id !== search.id)); } catch (exception) { window.alert(exception instanceof Error ? exception.message : "حذف جست‌وجو ممکن نشد."); } }
  async function apply(o: Opportunity) {
    if (o.application_mode === "internal") {
      if (!user) { setAuthOpen(true); return; }
      if (!user.email_verified) { window.location.href = "/account"; return; }
      const consent = window.confirm("با تأیید، نام، ایمیل، اطلاعات مرتبط پروفایل، پیام و رزومه اصلی شما برای شرکت همین آگهی ارسال می‌شود. آیا موافقید؟");
      if (!consent) return;
      try {
        const response = await api<{ message: string }>(`/applications/${o.slug}`, { method: "POST", body: JSON.stringify({ consent_data_sharing: true }) });
  ...
  const findMatches = (germanLevel: string) => { setFilter("german_level", germanLevel.toUpperCase()); setFilter("sort", user ? "match" : "latest"); navigate("search"); };
  ...
  const navItems: Array<[View,string]> = [["search","فرصت‌ها"],["saved","ذخیره‌شده‌ها"],["applications","درخواست‌های من"],["guide","راهنمای رشته‌ها"],["eligibility","بررسی شرایط"],["tools","ابزارها"],["employer","پنل کارفرما"]];
  ...
  {view==="search"&&<SearchPage opportunities={opportunities} loading={loading} error={error} filters={filters} meta={meta} activeFilterCount={activeFilterCount} setFilter={setFilter} toggleFavorite={toggleFavorite} apply={apply} report={setReportingOpportunity} openDetails={openDetails} saveCurrentSearch={saveCurrentSearch} user={user} total={total} page={page} lastPage={lastPage} setPage={setPage} reload={loadOpportunities}/>}
  {view==="saved"&&<SavedPage opportunities={opportunities} searches={savedSearches} searchesLoading={savedSearchesLoading} loading={loading} error={error} toggleFavorite={toggleFavorite} apply={apply} report={setReportingOpportunity} openDetails={openDetails} runSearch={runSavedSearch} deleteSearch={deleteSavedSearch} total={total} page={page} lastPage={lastPage} setPage={setPage} navigate={navigate} reload={loadOpportunities}/>}
...
function SearchPage(p:any) { return <><BlueHero eyebrow="فرصت مناسب همین نزدیکی‌ست" title="جست‌وجوی آوسبیلدونگ" text="فرصت‌های منتشرشده را با رشته، شهر و شرایط خودت فیلتر کن." icon={<Search/>}/><section className="page-surface container search-surface"><div className="search-box"><Search/><input value={p.filters.q} onChange={(e)=>p.setFilter("q",e.target.value)} placeholder="عنوان رشته، شهر یا شرکت"/></div><div className="filter-chips"><select value={p.filters.city} onChange={(e)=>p.setFilter("city",e.target.value)}><option value="">همه شهرها</option>{p.meta.cities.map((x:string)=><option key={x}>{x}</option>)}</select><select value={p.filters.german_level} onChange={(e)=>p.setFilter("german_level",e.target.value)}><option value="">سطح زبان</option>{p.meta.german_levels.map((x:string)=><option key={x}>{x}</option>)}</select><select value={p.filters.category} onChange={(e)=>p.setFilter("category",e.target.value)}><option value="">همه رشته‌ها</option>{p.meta.categories.map((x:any)=><option key={x.id} value={x.slug}>{x.name_fa}</option>)}</select><select value={p.filters.sort} onChange={(e)=>p.setFilter("sort",e.target.value)}><option value="latest">جدیدترین</option><option value="start">نزدیک‌ترین شروع</option><option value="salary">بیشترین حقوق</option>{p.user&&<option value="match">بیشترین تطابق</option>}</select><label><input type="checkbox" checked={p.filters.international} onChange={(e)=>p.setFilter("international",e.target.checked)}/> پذیرش بین‌المللی</label></div><div className="results-head"><h2>{fa.format(p.total)} نتیجه پیدا شد</h2><button className="save-search-button" onClick={p.saveCurrentSearch}><Save/> ذخیره این جست‌وجو</button></div>{p.loading?<div className="cards-grid">{[1,2,3].map(x=><div className="skeleton-card" key={x}/>)}</div>:p.error?<div className="empty-state"><p>{p.error}</p><button onClick={p.reload}>تلاش دوباره</button></div>:p.opportunities.length===0?<div className="empty-state"><p>فرصتی مطابق فیلترهای فعلی پیدا نشد.</p></div>:<OpportunityCards {...p}/>}<Pagination {...p}/></section></>; }

function SavedPage(p:any) { return <><BlueHero eyebrow="برای مراجعه دوباره" title="علاقه‌مندی‌ها و جست‌وجوهای ذخیره‌شده" text="فرصت‌های نشان‌شده و فیلترهای مهمت را از یک‌جا مدیریت کن." icon={<Bookmark/>}/><section className="page-surface container saved-page"><div className="saved-section-head"><div><span>فیلترهای آماده</span><h2>جست‌وجوهای ذخیره‌شده</h2></div><button onClick={()=>p.navigate("search")}><Search/> جست‌وجوی تازه</button></div>{p.searchesLoading?<div className="saved-search-skeleton"/>:p.searches.length===0?<div className="saved-empty"><p>هنوز جست‌وجویی ذخیره نکرده‌اید.</p><span>در صفحه جست‌وجو فیلترها را انتخاب و «ذخیره این جست‌وجو» را بزنید.</span></div>:<div className="saved-search-grid">{p.searches.map((item:SavedSearch)=><article key={item.id}><div><h3>{item.name}</h3><p>{savedFilterSummary(item.filters)}</p></div><div><button onClick={()=>p.runSearch(item)}><Search/> اجرای جست‌وجو</button><button className="delete-saved-search" onClick={()=>p.deleteSearch(item)} aria-label={`حذف ${item.name}`}><Trash2/></button></div></article>)}</div>}<div className="saved-section-head favorites-heading"><div><span>{fa.format(p.total)} مورد</span><h2>فرصت‌های موردعلاقه</h2></div></div>{p.loading?<div className="cards-grid">{[1,2,3].map(x=><div className="skeleton-card" key={x}/>)}</div>:p.error?<div className="empty-state"><p>{p.error}</p><button onClick={p.reload}>تلاش دوباره</button></div>:p.opportunities.length===0?<div className="saved-empty"><p>هنوز فرصتی ذخیره نکرده‌اید.</p><button onClick={()=>p.navigate("search")}>مشاهده فرصت‌ها</button></div>:<OpportunityCards {...p}/>}<Pagination {...p}/></section></>; }

function savedFilterSummary(filters: SearchFilters): string { const parts = [filters.q&&`عبارت: ${filters.q}`, filters.city&&`شهر: ${filters.city}`, filters.german_level&&`زبان: ${filters.german_level.toUpperCase()}`, filters.category&&`رشته: ${filters.category}`, filters.international&&"پذیرش بین‌المللی"].filter(Boolean); return parts.length ? parts.join(" · ") : "همه فرصت‌ها"; }
function OpportunityCards(p:any) { return <div className="cards-grid">{p.opportunities.map((item:Opportunity)=><div className="card-wrap" key={item.id}><OpportunityCard opportunity={item} onFavorite={p.toggleFavorite} onApply={p.apply} onReport={p.report}/><button className="card-detail-link" onClick={()=>p.openDetails(item)}>مشاهده جزئیات کامل <ChevronLeft/></button></div>)}</div>; }
function Pagination(p:any) { return p.lastPage>1?<div className="pagination"><button disabled={p.page<=1} onClick={()=>p.setPage(p.page-1)}>قبلی</button><span>{fa.format(p.page)} از {fa.format(p.lastPage)}</span><button disabled={p.page>=p.lastPage} onClick={()=>p.setPage(p.page+1)}>بعدی</button></div>:null; }

function DetailsPage({opportunity,onBack,onFavorite,onApply,onReport}:any) { if(!opportunity)return <div className="empty-state container">فرصتی انتخاب نشده است.</div>; return <><section className="detail-hero"><div className="container"><button className="round-back" onClick={onBack}><ArrowLeft/></button><button className="round-save" onClick={()=>onFavorite(opportunity)}><Bookmark fill={opportunity.is_favorite?"currentColor":"none"}/></button><div className="detail-title"><AppIcon><BriefcaseBusiness/></AppIcon><div><h1 dir="ltr">{opportunity.title_de}</h1><p>{opportunity.employer_name}</p><span><MapPin/> {opportunity.city}{opportunity.state?` · ${opportunity.state}`:""}</span><span><CalendarDays/> شروع: {opportunity.start_date||"در آگهی مشخص نشده"}</span><span><BadgeEuro/> {opportunity.monthly_salary_from?`${fa.format(opportunity.monthly_salary_from)} €`:"حقوق در آگهی مشخص نشده"}</span></div></div><div className="friendly-illustration compact"><GraduationCap/></div></div></section><section className="page-surface container detail-content"><article className="content-card"><h2>درباره دوره</h2><p>{opportunity.description_fa}</p></article><article className="content-card"><h2>شرایط ثبت‌شده در آگهی</h2><ul className="check-list"><li>زبان آلمانی {opportunity.required_german_level}</li>{opportunity.education_requirement&&<li>{opportunity.education_requirement}</li>}<li>{opportunity.accepts_international?"پذیرش متقاضی بین‌المللی اعلام شده":"پذیرش بین‌المللی در منبع تأیید نشده"}</li></ul></article>{opportunity.skills.length>0&&<article className="content-card"><h2>مهارت‌های درج‌شده</h2><div className="tag-list">{opportunity.skills.map((x:string)=><span key={x}>{x}</span>)}</div></article>}{opportunity.application_mode==="internal"&&<p className="application-consent-note">در مرحله بعد، ارسال نام، ایمیل، اطلاعات مرتبط پروفایل و رزومه اصلی به شرکت این آگهی را جداگانه تأیید می‌کنید. <a href="/privacy">جزئیات حریم خصوصی</a></p>}<button className="primary-button wide-action" onClick={()=>onApply(opportunity)}>{opportunity.application_mode==="internal"?"ارسال درخواست مستقیم":"بازکردن صفحه درخواست کارفرما"} <ArrowLeft/></button><button type="button" className="detail-report-button" onClick={()=>onReport(opportunity)}>گزارش کلاهبرداری، لینک خراب، اطلاعات اشتباه یا فرصت منقضی</button></section></>; }
...
function BottomNav({view,navigate,openProfile}:any){const items:Array<[View,string,ReactNode]>=[["home","خانه",<House/>],["search","جست‌وجو",<Search/>],["saved","ذخیره‌ها",<Bookmark/>],["applications","درخواست‌ها",<FileText/>]];return <nav className="bottom-nav">{items.map(([k,l,i])=><button key={k} className={view===k?"active":""} onClick={()=>navigate(k)}>{i}<span>{l}</span></button>)}<button onClick={openProfile}><UserRound/><span>پروفایل</span></button></nav>}
function SiteFooter(){const links:Array<[LegalPageKey,string]>=[["privacy","حریم خصوصی"],["terms","شرایط استفاده"],["impressum","Impressum"],["resume-retention","نگهداری رزومه"]];return <footer className="site-footer"><div className="container"><span>© {new Date().getFullYear()} Ausbildung Match</span><nav aria-label="صفحات قانونی">{links.map(([key,label])=><a key={key} href={legalPaths[key]}>{label}</a>)}</nav></div></footer>}
