...
import { ArrowLeft, BadgeEuro, Bookmark, BriefcaseBusiness, Building2, CalendarDays, Check, CheckCircle2, ChevronLeft, FileCheck2, FileText, GraduationCap, House, LayoutGrid, Lightbulb, ListChecks, LogOut, MapPin, Menu, Mic, Search, Sparkles, UserRound, Wrench, X } from "lucide-react";
import { api } from "./api";
...
type Assessment = { age: string; education: string; language: string; experience: string };
function EligibilityPage({findMatches}:any){const [values,setValues]=useState<Assessment>({age:"",education:"",language:"",experience:""});const [result,setResult]=useState<number|null>(null);const points={age:{young:25,mid:22,senior:15},education:{diploma:20,associate:22,bachelor:25},language:{a2:12,b1:22,b2:28,c1:30},experience:{none:8,junior:14,experienced:20}} as any;const score=result??0;const set=(key:keyof Assessment,value:string)=>setValues(current=>({...current,[key]:value}));const submit=(event:FormEvent)=>{event.preventDefault();setResult(points.age[values.age]+points.education[values.education]+points.language[values.language]+points.experience[values.experience])};return <><BlueHero eyebrow="ارزیابی شفاف و محاسباتی" title="بررسی آمادگی اولیه" text="نتیجه بر اساس پاسخ‌های خودت محاسبه می‌شود و جایگزین ارزیابی رسمی نیست." icon={<ListChecks/>}/><section className="page-surface container eligibility-card">{result!==null?<><div className="result-grid"><div><h2>امتیاز آمادگی اولیه</h2><p>{score>=75?"آمادگی اولیه خوب است":score>=55?"چند بخش نیاز به تقویت دارد":"پیش از اقدام، روی پیش‌نیازها کار کن"}</p><div className="score-ring" style={{background:`radial-gradient(circle,#fff 57%,transparent 59%),conic-gradient(var(--blue) 0 ${score}%,#e8eef5 ${score}%)`}}>{fa.format(score)}٪</div></div><div className="result-visual"><CheckCircle2/><b>این امتیاز تضمین پذیرش یا ویزا نیست.</b></div></div><ul className="result-list"><li><Check/> امتیاز زبان: {fa.format(points.language[values.language])} از ۳۰</li><li><Check/> امتیاز تحصیلات: {fa.format(points.education[values.education])} از ۲۵</li><li className={values.language==="a2"||values.language==="b1"?"warning":""}>سطح انتخابی زبان: {values.language.toUpperCase()}</li></ul>{(values.language==="a2"||values.language==="b1")&&<div className="tip-box"><Lightbulb/><div><b>تقویت زبان می‌تواند فرصت‌های بیشتری باز کند</b><span>سطح دقیق موردنیاز را در هر آگهی جداگانه بررسی کن.</span></div></div>}<button className="primary-button wide-action" onClick={()=>findMatches(values.language)}>مشاهده فرصت‌های متناسب با زبان</button><button className="secondary-button wide-action" onClick={()=>setResult(null)}>ویرایش پاسخ‌ها</button></>:<form className="assessment-form" onSubmit={submit}><h2>اطلاعات اولیه</h2><label>بازه سنی<select required value={values.age} onChange={e=>set("age",e.target.value)}><option value="">انتخاب کنید</option><option value="young">۱۸ تا ۲۴ سال</option><option value="mid">۲۵ تا ۳۴ سال</option><option value="senior">۳۵ سال به بالا</option></select></label><label>آخرین مدرک تحصیلی<select required value={values.education} onChange={e=>set("education",e.target.value)}><option value="">انتخاب کنید</option><option value="diploma">دیپلم</option><option value="associate">کاردانی</option><option value="bachelor">کارشناسی یا بالاتر</option></select></label><label>سطح زبان آلمانی<select required value={values.language} onChange={e=>set("language",e.target.value)}><option value="">انتخاب کنید</option><option value="a2">A2</option><option value="b1">B1</option><option value="b2">B2</option><option value="c1">C1</option></select></label><label>سابقه مرتبط<select required value={values.experience} onChange={e=>set("experience",e.target.value)}><option value="">انتخاب کنید</option><option value="none">کمتر از یک سال</option><option value="junior">۱ تا ۳ سال</option><option value="experienced">بیشتر از ۳ سال</option></select></label><button className="primary-button wide-action">محاسبه نتیجه</button></form>}</section></>}
